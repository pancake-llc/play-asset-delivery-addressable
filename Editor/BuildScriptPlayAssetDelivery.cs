...
namespace AddressablesPlayAssetDelivery.Editor
{
    ...
    public class BuildScriptPlayAssetDelivery : BuildScriptPackedMode
    {
        ...
        string CreateAssetPackDirectory(string assetPackName)
        {
            ...
            if (!AssetDatabase.IsValidFolder(src)) AssetDatabase.CreateFolder(path, "src");
            if (!AssetDatabase.IsValidFolder(main)) AssetDatabase.CreateFolder(src, "main");
            if (!AssetDatabase.IsValidFolder(assets)) AssetDatabase.CreateFolder(main, "assets");
            return path;
        }
